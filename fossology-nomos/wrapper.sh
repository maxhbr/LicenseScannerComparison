...
RUN set -x\
 ...
 && echo '    exec find \$1 -type f -exec echo -n "{};" \\; -exec /fossology/src/nomos/agent/nomos -J {} \\; -exec echo "" \\; 2>/dev/null' >>/entrypoint.sh \
 && echo 'else' >>/entrypoint.sh \
 && echo '    exec /fossology/src/nomos/agent/nomos \$@ 2>/dev/null' >>/entrypoint.sh \
